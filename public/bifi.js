...
function bfi_init(options = null) {
    ...
    for (let i = 0; i < total; i++) {
        ...
        if (input.hasAttribute('multiple')) {
            ...
            label = 'Per <span>Klick</span> mehrere Dateien auswählen oder Drag-and-drop verwenden ';
        } else {
            input.classList.add('bfi-converted');
            label = 'Per <span>Klick</span> Datei auswählen oder Drag-and-drop verwenden ';
        }
...
document.addEventListener('change', e => {
    if (e.target.classList.contains('bfi-converted')) {
        ...
        if (e.target.files.length) {
            ...
            container.insertAdjacentHTML('beforeend', `<div class="bfi-file"><span class="bfi-clear" tabindex="0">&#10008;</span>${file}<br><i>${size}</i></div>`);
        } else {
    ...
    if (e.target.classList.contains('bfi-converted-multi')) {
        ...
        if (e.target.files.length) {
            ...
            let fileCount = '1 Datei';
            if (files.length > 1) fileCount = `${files.length} Dateien`;
            container.querySelector('.bfi-label-selected').innerHTML = `${fileCount} ausgewählt. <span class="bfi-clear" tabindex="0">&#10008;</span>`;
            files.forEach(file => {
